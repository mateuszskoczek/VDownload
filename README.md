...
| `download <link> [options]`          | Download video<br><br>Options:<br>`--option_preset=<name>` (apply options from specified option preset; other options specified in this command overwrite options from preset)<br>`--onlyvideo` (download only video [Y])<br>`--onlyaudio` (download only audio [Y])<br>`--vstream=<id>` (download video stream with specified id; best if not specified [Y])<br>`--astream=<id>` (download audio stream with specified id; best if not specified [Y])<br>`--output_path=<path>` (output file path [Y])<br>`--filename=<filename>` (filename; template [Y])<br>`--video_ext=<extension>` (video file extension [Y])<br>`--audio_ext=<extension>` (audio file extension [Y])<br>`--trim_start=<time>` (trim video on the beginning [Y])<br>`--trim_end=<time>` (trim video on the end [Y])<br>`--videos=<id>` (download videos from playlist with specified ids; semicolon-separated [YP]) |
...
`vdownload download https://www.youtube.com/watch?v=dQw4w9WgXcQ --onlyvideo --vstream=5 --filename="absolutely nothing suspicious here"`
...
| `max_files_downloading_tasks` | Maximum number of files downloaded asynchronously (some downloads are not affected) |
...
| ------------- | --------------------------------- |
| `%title%`     | Video title [Y&#124;T]            |
| `%author%`    | Video author [Y&#124;T]           |
| `%author_id%` | Video author id [Y&#124;T]        |
| `%pub_date%`  | Video publication date [Y&#124;T] |
| `%id%`        | Video ID [Y&#124;T]               |
| `%act_date%`  | Actual date [Y&#124;T]            |
| `%duration%`  | Video duration [Y&#124;T]         |
| `%views%`     | Video views [Y&#124;T]            |
| `%likes%`     | Video rating (likes) [Y]          |
| `%dislikes%`  | Video rating (dislikes) [Y]       |
...
- AsyncEnumerator [[LINK]](https://github.com/Dasync/AsyncEnumerable)
